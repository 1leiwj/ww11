				...
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="失物类型" prop="shiwuleixing">
							<el-input class="list_inp" v-model="form.shiwuleixing" placeholder="失物类型"
					...
					<el-col :span="24">
						<el-form-item label="认领人" prop="renlingren">
							<el-input class="list_inp" v-model="form.renlingren" placeholder="认领人"
								 type="text" 								:readonly="!isAdd||disabledForm.renlingren?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="认领时间" prop="renlingshijian">
							<el-date-picker
								class="list_date"
								v-model="form.renlingshijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.renlingshijian?true:false"
								placeholder="请选择认领时间" />
						</el-form-item>
					</el-col>
					<el-col :span="24">
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.renlingshijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='renlingren'){
					form.value.renlingren = row[x];
					disabledForm.value.renlingren = true;
					continue;
				}
				if(x=='renlingshijian'){
					form.value.renlingshijian = row[x];
					disabledForm.value.renlingshijian = true;
					continue;
				}
			}
			if(row){
		...
		context?.$http({
			...
			if(json.hasOwnProperty('xuehao')&& context?.$toolUtil.storageGet("role")!="管理员"){
				form.value.renlingren = json.xuehao
				disabledForm.value.renlingren = true;
			}
		})
	}
