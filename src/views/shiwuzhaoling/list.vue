	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.renlingzhuangtai" 
								placeholder="认领状态"
								>
								<el-option v-for="item in renlingzhuangtaiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('shiwuzhaoling','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('shiwuzhaoling','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('shiwuzhaoling','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('shiwuzhaoling','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('shiwuzhaoling','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('shiwuzhaoling','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('shiwuzhaoling','认领')" type="success" @click="renlingjiluCrossAddOrUpdateHandler(scope.row,'cross','是','renlingzhuangtai','已认领','已认领,未认领'.split(',')[0])">认领</el-button>
						<el-button v-if="btnAuth('shiwuzhaoling','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<renlingjiluFormModel ref="renlingjiluFormModelRef" @formModelChange="formModelChange"></renlingjiluFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.renlingzhuangtai&&searchQuery.value.renlingzhuangtai!=''){
			params['renlingzhuangtai'] = searchQuery.value.renlingzhuangtai
		}
          if(searchQuery.value.sfsh && searchQuery.value.sfsh!=''){
            params['sfsh'] = searchQuery.value.sfsh
          }
	...
	const getrenlingzhuangtaiLists = () => {
		renlingzhuangtaiLists.value = '已认领,未认领'.split(',')
	}
	getrenlingzhuangtaiLists()
	const searchClick = () => {
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussshiwuzhaoling?refid=' + id)
	}
	import renlingjiluFormModel from '@/views/renlingjilu/formModel'
	const renlingjiluFormModelRef = ref(null)
    const renlingjiluCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			renlingjiluFormModelRef.value.init(row.id,'cross','认领',row,'shiwuzhaoling',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
